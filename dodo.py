...
def task_validate_example_files():
  ...
  for example in examples:
    ...
    yield {
      'name': name,
      'file_dep': [schema.json_schema_path for schema in example.schemas] + example.examples + [Path(SOURCE_PATH, "schema_to_json.py")],
...
def task_generate_markdown():
  ...
  for example in examples:
    ...
    yield {
      ...
      'file_dep': [schema.path for schema in example.schemas] + [template.path for template in example.doc_templates] + [Path(SOURCE_PATH, "docs", "grid_table.py")],
      'task_dep': [f"validate_schemas:{name}"],
      'actions': [
    ...
    for example in examples:
        ...
        yield {
          ...
          'file_dep': [schema.path for schema in example.cpp_schemas] +
                      ...
                      [CORE_SCHEMA_PATH,
                      BASE_META_SCHEMA_PATH,
                      Path(SOURCE_PATH, "header_entries.py")],
...
def task_generate_web_docs():
  ...
  for example in examples:
    ...
    yield {
      ...
      'file_dep': [schema.path for schema in example.schemas] + [template.path for template in example.doc_templates] + [Path(SOURCE_PATH, "docs", "mkdocs_web.py")],
