...
class HugoWeb:  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self, lattice):
        ...
        self.author = ""
        self.has_logo = False
        self.colors = {"primary": "#30638E", "secondary": "#FFA630", "menu_text": "#000"}
    ...
    def make_pages(self):
        ...
        # Create scss file
        render_template(
            os.path.join(os.path.dirname(__file__), "scss_template.scss.j2"),
        ...
        HugoWeb.make_page(
            landing_page_path,
            {"title": self.title, "type": "landing", "description": self.description},
        ...
        self.make_main_menu_page(
            self.specifications_directory_path,
            "Specifications",
    ...
    def make_specification_pages(self):  # pylint: disable=missing-function-docstring
        ...
        # Process templates
        for template in self.specification_templates:
            template.markdown_output_path = os.path.join(
                self.specifications_directory_path, f"{get_file_basename(template.path, depth=2)}.pdc"
            )
    ...
    def make_examples_page(self):  # pylint: disable=missing-function-docstring
        ...
        for example in self.lattice.examples:
            ...
            web_links = {}
            for fmt in formats:
                output_path[fmt] = os.path.join(example_assets_directory, f"{file_base_name}.{fmt}")
                references[reference_counter] = (
                    f"/{self.git_repo_name}/" f"{os.path.relpath(output_path[fmt], self.static_directory_path)}"
                )
    ...
    def make_main_menu_page(
        self, page_dir_path, title, content="", schema_path=None, content_type=None, content_path=None
    ):
        front_matter = {
            "title": title,
            "linkTitle": title,
            "weight": 1,
            "menu": {"main": {"weight": self.main_menu_item_counter}},
        }
    ...
    def build(self):
        ...
        if not os.path.exists(os.path.join(self.build_directory, "go.mod")):
            subprocess.run(
                ["hugo", "mod", "init", os.path.relpath(self.docs_source_directory).replace("\\", "/")],
        ...
        if not os.path.exists(os.path.join(self.build_directory, "go.sum")):
            subprocess.run(
                ["hugo", "mod", "get", r"github.com/google/docsy@v0.6.0"],
                cwd=self.build_directory,
                check=True,
                shell=shell,
            )
    ...
    def make_hugo_config(self):  # pylint: disable=missing-function-docstring
        return {
            ...
            "enableGitInfo": True,
            "module": {
                "hugoVersion": {"extended": True, "min": "0.73.0"},
                "imports": [
                    {"path": "github.com/google/docsy", "disable": False},
                    {"path": "github.com/google/docsy/dependencies", "disable": False},
                ],
            },
            "params": {
                ...
                "ui": {"navbar_logo": self.has_logo, "breadcrumb_disable": True},
            },
            "security": {
    ...
    def make_npm_package_json():  # pylint: disable=missing-function-docstring
        return {
            ...
            "description": "",
            "dependencies": {},
            "devDependencies": {"autoprefixer": "^10.4.0", "postcss": "^8.3.7", "postcss-cli": "^9.0.2"},
        }
