...
# TODO: find missing required data elements in template-derived Data Groups, and Array constraints like
#       [Numeric][1..] where I guess the size value has been moved to the Constraints list.
...
class ConstraintsPattern(RootModel):
    root: Annotated[
        str,
        StringConstraints(
            pattern=rf'^((>|>=|<=|<)(([-+]?[0-9]*\.?[0-9]+([eE][-+]?[0-9]+)?)))|(%(([-+]?[0-9]*\.?[0-9]+([eE][-+]?[0-9]+)?)))|(\[([-+]?[0-9]*\.?[0-9]+([eE][-+]?[0-9]+)?)(, ?([-+]?[0-9]*\.?[0-9]+([eE][-+]?[0-9]+)?))*\])|(({schema._data_element_names.pattern.pattern})=(((([-+]?[0-9]*\.?[0-9]+([eE][-+]?[0-9]+)?))|(".*")|(([A-Z]([A-Z]|[0-9])*)(_([A-Z]|[0-9])+)*)|(True|False))))|(:[A-Z]([A-Z]|[a-z]|[0-9])*:)|(([a-z][a-z,0-9]*)(_([a-z,0-9])+)*\(([A-Z]([A-Z]|[0-9])*)(_([A-Z]|[0-9])+)*(, ?([A-Z]([A-Z]|[0-9])*)(_([A-Z]|[0-9])+)*)*\))|(\[(\d*)\.\.(\d*)\])|(".*")$'  # noqa: E501
        ),
    ]
...
class ElementBasedConditionalRequiredPattern(RootModel):
    ...
    root: Annotated[
        str,
        StringConstraints(
            pattern=rf"if ({schema._data_element_names.pattern.pattern})(!=|=)({schema.BooleanType.value_pattern})"
        ),
    ]
...
class EnumValueBasedConditionalRequiredPattern(RootModel):
    ...
    root: Annotated[
        str,
        StringConstraints(
            pattern=rf"if ({schema._data_element_names.pattern.pattern})(!=|=)({schema.EnumerationType.value_pattern})"
        ),
    ]
...
class RequiredAttribute(RootModel):
    root: Union[
        bool,
        ElementBasedConditionalRequiredPattern,
        EnumValueBasedConditionalRequiredPattern,
        ExclusionaryRequiredPattern,
    ]
...
class Enumerants(RootModel):
    root: Dict[
        Annotated[str, StringConstraints(pattern=schema.EnumerationType.value_pattern.pattern)], EnumeratorAttributes
    ]
...
class DataTypePattern(RootModel):
    root: Annotated[
        str,
        StringConstraints(
            pattern=r"^(((Integer|Numeric|Boolean|String|Pattern)|(UUID|Date|Timestamp|Version)|\{([A-Z]([A-Z]|[a-z]|[0-9])*)\}|<[A-Z]([A-Z]|[a-z]|[0-9])*>|:[A-Z]([A-Z]|[a-z]|[0-9])*:))|(\((((Integer|Numeric|Boolean|String|Pattern)|(UUID|Date|Timestamp|Version)|\{([A-Z]([A-Z]|[a-z]|[0-9])*)\}|<[A-Z]([A-Z]|[a-z]|[0-9])*>|:[A-Z]([A-Z]|[a-z]|[0-9])*:))(,\s*((Integer|Numeric|Boolean|String|Pattern)|(UUID|Date|Timestamp|Version)|\{([A-Z]([A-Z]|[a-z]|[0-9])*)\}|<[A-Z]([A-Z]|[a-z]|[0-9])*>|:[A-Z]([A-Z]|[a-z]|[0-9])*:))+\))|(\[([A-Z]([A-Z]|[a-z]|[0-9])*|\{([A-Z]([A-Z]|[a-z]|[0-9])*)\}|<[A-Z]([A-Z]|[a-z]|[0-9])*>)\])$"  # noqa: E501
...
class DataGroup(BaseModel):
    ...
    Data_Elements: Dict[
        Annotated[str, StringConstraints(pattern=schema.DataElement.pattern.pattern)], DataElementAttributes
    ] = Field(..., alias="Data Elements")
...
class DataGroupTemplate(BaseModel):
    ...
    Unit_System: Optional[str] = Field(None, alias="Unit System")
    Required_Data_Types: Optional[List[str]] = Field(None, alias="Required Data Types")
    Data_Elements_Required: Optional[bool] = Field(None, alias="Data Elements Required")


class Item(RootModel):
    root: Union[DataType, StringType, DataGroup, DataGroupTemplate, Enumeration, Meta] = Field(
...
class LatticeSchema(BaseModel):
    ...
    def find_data_group_template(self) -> Self:
        ...
        for _, datagroup in [
            (k, v)
            for (k, v) in self.lattice_schema.items()
            if isinstance(v.root, DataGroup) and v.root.Data_Group_Template
        ]:
            matched = False
            # Iterate over all the possible template names that could be referenced by the group's DGT attribute
            for template_name, _ in [
                (k, v) for (k, v) in self.lattice_schema.items() if isinstance(v.root, DataGroupTemplate)
            ] + referenceable_templates:
    ...
    @model_validator(mode="after")
    def validate_data_element_units(self) -> Self:  # noqa: PLR0912 too-many-branches
                                        ...
                                        if isinstance(template.root, DataGroupTemplate):
                                            if datagroup.root.Data_Group_Template == template_name:
                                                if template.root.Unit_System:
                                                    if (
                                                        attributes.Units
                                                        not in self.model_unit_systems[template.root.Unit_System]
                                                    ):
                                                        raise Exception(
                                                            f"Unit {attributes.Units} for element {name} was not "
                                                            f"found in Unit System {template.root.Unit_System}."
                                                        )
                                                elif attributes.Units not in [u.value for u in StandardUnits]:
                                                    raise Exception(
                                                        f"Unit {attributes.Units} for element {name} was not "
                                                        f"found in Standard Units."
                                                    )
                    elif datagroup.root.Unit_System:
                        if attributes.Units not in self.model_unit_systems[datagroup.root.Unit_System]:
                            raise Exception(
                                f"Unit {attributes.Units} for element {name} "
                                f"was not found in Unit System {datagroup.root.Unit_System}."
                            )
                    elif attributes.Units not in [u.value for u in StandardUnits]:
